...
namespace NMEA
{
    ...
    public static class NMEAParser
    {
        ...
        public static readonly int MaxFullSentenceLength = 255;//82;
        public static readonly int MaxSentenceLength = 252;//79;
        ...
        private static Dictionary<SentenceIdentifiers, string> SentencesFormats =
            new Dictionary<SentenceIdentifiers, string>() { { SentenceIdentifiers.AAM, "A=Arrival circled entered|V=V,A=Perpendicular passed at way point|V=V,x.x,N=nm|K=km,c--c" },
                                                            ...
                                                            { SentenceIdentifiers.RMC, "hhmmss.ss,A,llll.ll,N=North|S=South,yyyyy.yy,E=East|W=West,x.x,x.x,ddmmyy,x.x,a,a" },
        ...
        private static Dictionary<ManufacturerCodes, Dictionary<string, string>> ProprietarySentencesFormats =
            new Dictionary<ManufacturerCodes, Dictionary<string, string>>()
            {
                ...
                {
                    ManufacturerCodes.SHT, // RI SHTIL
                    new Dictionary<string, string>()
                    {
                        {"INC", "x.x,x.x"}, /// target inclination X and Y axis
                        {"ACC", "x.x,x.x"}, /// target acceleration X and Y axis
                        {"INC3", "x.x,x.x,x.x"}, /// target inclination X,Y and Z axis
                        {"ACC3", "x.x,x.x,x.x"}, /// target acceleration X,Y, and Z axis
                        {"TMP", "x.x,C=C|K=K|F=F"}, /// target temperature                                                                   
                    }
                },
        ...
        private static string FormatLatitude(double x)
        {
            ...
            sb.Append(minutes.ToString("00.00", CultureInfo.InvariantCulture).TrimEnd(new char['0']));
        ...
        private static string FormatDegree(double x)
        {
            ...
            sb.Append(degrees.ToString("000"));
            sb.Append(minutes.ToString("00", CultureInfo.InvariantCulture).TrimEnd(new char['0']));
